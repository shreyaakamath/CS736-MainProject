...
class Instance:
	...
	def __init__(self, id, type, active, start_time,perf_array, end_time = 0, total_time = 0, total_time_computation = 0, total_work = 0, avg_perf = 0):
		...
		self.perf_first = perf_array[0][0] #Stores perf for 1st quantum for 1st HLA
	def __repr__(self):
...
class Customer:
	...
	def config_loader(self):
		...
		self.strategy, self.T, self.units, self.A, self.B, self.m, self.mu, self.c, self.cu, self.unsatisfiability_quotient = [float(x) for x in lines[0].split(",")]
		self.T = int(self.T)
		self.A = int(self.A)
		self.B = int(self.B)
		self.unsatisfiability_quotient = int(self.unsatisfiability_quotient)
		...
		for line in lines:
			...
			mean = [float(record[2]), float(record[3]), float(record[4]), float(record[5]), float(record[6])]
			stddev = [float(record[7]), float(record[8]), float(record[9]), float(record[10]), float(record[11])]

			instType_obj = Instance_type(rank_count, float(record[1]), mean, stddev)
	...
	def launch_instance(self,instance_id, time):
		...
		if self.collaborator == False or time == 0:
			...
			for i in range(0,self.T):
				# Generate the 5-D perf array instead of a number now
				std_normal = gen_std_normal()
				perf = []
				for j in range(0,5):
					perf.append((self.instanceTypes[whichRank].stddev[j] + std_normal) + self.instanceTypes[whichRank].mean[j])	
	...
	def kill_instance(self,instance_id, time, naive_instance = False, collaborator_instance = False):
		...
		if collaborator_instance == False:
			...
			while(t < instance_obj.total_time_computation):
				...
				if(t == 0):
					instance_obj.total_work += (self.units - self.m) * instance_obj.perf_array[i][0] #Calculating work on 1st HL SLA
					t += self.units - self.m
				else:
					instance_obj.total_work += self.units * instance_obj.perf_array[i][0] #Calculating work on 1st HL SLA
					t += self.units
			...
			instance_obj.total_time_computation = instance_obj.total_time - self.m 
			instance_obj.active = 0
			instance_obj.total_work = (self.units) *instance_obj.perf_array[0][0] #Calculating work on 1st HL SLA


	def calculate_delta(self, gold_standard, current_standard):

		unsatisfiability_quotient = 0 

		for index in range(0,len(gold_standard)):

			if current_standard[index] < gold_standard[index]:
				unsatisfiability_quotient += 1

		return unsatisfiability_quotient
	...
	def simulate(self):
		...
		cur_perf = [0,0,0,0,0] #Will now be an array
		num_active = 0
		first_avg = [0,0,0,0,0] #Will now be an array
		stddev = 0
		#delta = 0 #Needs to change now 
		num_migrated = 0
		...
		for i in range(0,self.A+self.B):
			...
			num_instances += 1
			for index in range(0, len(self.instances[i].perf_array[0])): 
				first_avg[index] += self.instances[i].perf_array[0][index] #This is the first average for Performance based strategies

		if (self.strategy == "CPU_OPREP"):
			first_avg = [0,0,0,0,0]
			for i in range(0,self.num_instance_types):
				for index in range(0,len(self.instanceTypes[i].mean)):
					first_avg[index] += self.instanceTypes[i].mean[index] #This is the first average for CPU based strategies
			first_avg = [x/self.num_instance_types for x in first_avg]
			#first_avg = first_avg / self.num_instance_types
		else:
			first_avg = [x/(self.A +self.B) for x in first_avg]
		...
		if (self.T > 0 and self.B > 0):
			if (self.strategy == "CPU"):
				...
				self.instances = sorted(self.instances, key = operator.attrgetter('perf_first'), reverse = True) #Sorting based on 1st HL-SLA
		...
		if (self.strategy in ['CPU_OPREP', 'UPFRONT_OPREP']):
			...
			for i in range(1, self.T-1):
				time += self.units
				#delta = self.mu * (self.m/float(self.units)) / (self.T - i) #This needs to change
				num_migrated = 0
				...
				for j in range(0, num_instances):
					...
					if (self.instances[j].active == 1):
						...
						if (self.calculate_delta(first_avg,cur_perf) > self.unsatisfiability_quotient):
...
if __name__ == "__main__":
	...
	customer = Customer("hla-config", True)
	customer.simulate()
